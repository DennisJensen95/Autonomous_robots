...
double visionpar[10];
double laserpar[10];
double calibrated_linesensor_values[7];
...
void calib_linesensor(symTableElement *linesensor, double *calib, int black, int white);
...
int main()
{
    int running,n=0,arg,time=0,black=52, white=58;
    double dist=0,angle=0,speed=0.2,acceleration=0.5,angular_acceleration=1;
    int w_line = 0;
...
/* Create endpoint */
    ...
    mission.oldstate=-1;
    mot.k_line_sensor[0] = 1;
    mot.k_line_sensor[1] = 0.5;
    mot.k_line_sensor[2] = 0.25;
    mot.k_line_sensor[3] = 0;
    mot.k_line_sensor[4] = 0.25;
    mot.k_line_sensor[5] = 0.5;
    mot.k_line_sensor[6] = 1;
    ...
    while (running){
        ...
        calib_linesensor(linesensor, calibrated_linesensor_values, black, white);

        // Find the lowest or highest calibration value for the linesensors
        mot.lowest_linesensor_index = min_max(calibrated_linesensor_values, w_line);
...
//  printf("laser %f\n", laserpar[3]);
        ...
        fprintf(fp_line, "%d %d %d %d %d %d %d\n", linesensor->data[0], linesensor->data[1], linesensor->data[2],
                linesensor->data[3], linesensor->data[4], linesensor->data[5], linesensor->data[6]);
//        fprintf(fp_line, "%f %f %f %f %f %f %f\n", calibrated_linesensor_values[0], calibrated_linesensor_values[1],
//                calibrated_linesensor_values[2], calibrated_linesensor_values[3],
//                calibrated_linesensor_values[4], calibrated_linesensor_values[5],
//                calibrated_linesensor_values[6]);
    ...
    switch (p->curcmd){
        ...
        case mot_fwl:
            ...
            if (fabs((p->right_pos+p->left_pos)/2 - p->startpos) > p->dist){
                ...
                p->motorspeed_r = speed_step;
                p->motorspeed_l = speed_step + 0.25 * p->k_line_sensor[p->lowest_linesensor_index];
            } else {
                p->motorspeed_r = speed_step + 0.25 * p->k_line_sensor[p->lowest_linesensor_index];;
                p->motorspeed_l = speed_step;
            }
            break;
    }
...
void calib_linesensor(symTableElement *linesensor, double  *calib_values, int black, int white) {
    int i;
    for (i = 0; i < 7; i++) {
        if (linesensor->data[i] < black) {
            calib_values[i] = 0;
        } else {
            calib_values[i] =  (double) (linesensor->data[i] - black)/(white-black);
        }
    }
}
