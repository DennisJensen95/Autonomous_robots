...
//        competition_track(&mot, &odo, &mission, &det, &rstate);
        ...
        */
        ...
        switch (mission.state) {
            ...
            case ms_fwd:
                if (fwd(rstate.dist, mot.speed_step, mission.time))  mission.functions += 1;
                break;
            ...
            case ms_fwl:
                if (fwl(rstate.dist,speed,mission.time))  mission.functions += 1;
                break;
            ...
            case ms_turn_180:
                angle = 180/180*M_PI;
                if (turn(angle, mot.speed_step, mission.time)) mission.functions += 1;
                break;
        }
        /*  end of mission  */
...
//Detect fork with branch <90deg , assume step change in line sensor delta_com=0.7
int detect_negative_fork(double com_prev, double com_new){

    return (fabs(com_new) >= fabs(com_prev+0.7));
}
